...
import {
  ChangeRequestTypeGridModel,
  ChangeRequestTypeViewModel,
} from "./../types";
...
export class ProductComponent implements OnInit {
      ...
      [
        new CustomAction({
          ...
          callback: (row: Product) => {
            that.model.Status;
            that.productService.createOrUpdate(that.model).subscribe(
              (response) => {
    ...
    return new FluentTableConfig({
      columns: [
        ...
        },
        {
          ...
          type: "action",
          handler(row: Product) {
            that.productService.get(row.Id).subscribe((x: any) => {
              that.model = x;
  ...
  summonDialog = () => {
    ...
    this.dynamicService.addComponent(
      this.viewContainer,
      ModalFormComponent,
      [
  ...
  getModalConfig(
    model: Product,
    onSave?: (result: LookupViewModel) => void
  ): ModalFormConfig {
    const that = this;
